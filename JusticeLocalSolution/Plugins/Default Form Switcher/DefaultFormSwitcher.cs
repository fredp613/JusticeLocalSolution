...
namespace JusticeLocalSolution.Plugins
{
    ...
    //public class DefaultFormSwitcherPluginRetrieveMultiple : IPlugin
    //{
    ...
    //            foreach (Entity entity in targets.Entities)
    //            {
    //                var dfs = new DefaultFormSwitcher(service, tracingService, entity.LogicalName);
    //                dfs.toggleDefaultFormXML(entity, context);
    //            }
...
namespace FredPearson.Common
{
    class DefaultFormSwitcher
    {
        ...
        public void toggleDefaultFormXML(Entity entity, IPluginExecutionContext context)
        {
            
            //get the objecttypecode in order to get the lastviewedformxml
            var meta = new MetadataHelper(_service, entity);
            ...
            if (objecttypecode != null)
            {
                ...
                var settings = _service.RetrieveMultiple(query).Entities;
                ...
                var switcherEntityResults = _service.RetrieveMultiple(switcherEntityQuery).Entities;
                ...
                if (switcherEntityResults.Count > 0)
                {
                    ...
                    if (!(settings == null || settings.Count != 1))
                    {
                        ...
                        _tracingService.Trace("object type code is: {0}--last viewed for xml is {1}", objecttypecode, lastViewedFormXml);
                    }
                }
